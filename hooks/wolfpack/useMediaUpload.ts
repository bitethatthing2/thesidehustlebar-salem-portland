...
interface UseMediaUploadReturn {
  posting: boolean;
  uploadToSupabase: (file: Blob, fileName: string, recordingMode: 'photo' | 'video') => Promise<string>;
...
export function useMediaUpload(): UseMediaUploadReturn {
  ...
  const uploadToSupabase = useCallback(async (file: Blob, fileName: string, recordingMode: 'photo' | 'video') => {
  ...
  const createPost = useCallback(async ({ capturedMedia, caption, recordingMode, recordingTime = 0 }: CreatePostParams) => {
    if (!capturedMedia) {
      toast({
        title: 'Error',
    ...
    try {
      ...
      const mediaUrl = await uploadToSupabase(capturedMedia, fileName, recordingMode);
