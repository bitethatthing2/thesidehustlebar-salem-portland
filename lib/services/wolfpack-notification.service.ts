...
import { createNotification, NotificationCreators } from '@/lib/notifications/create-notification';
...
export class WolfpackNotificationService {
  ...
  /**
   * Send notification when someone comments on a video
   */
  ...
  static async hasNotificationsEnabled(
    userId: string, 
    notificationType: 'likes' | 'comments' | 'follows' | 'mentions' | 'new_videos'
  ): Promise<boolean> {
    try {
